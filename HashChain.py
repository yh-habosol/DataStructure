...
class HashChaining:
    ...
    def set(self, key, value=None):
        if self.H[self.find_slot(key)].search(key) ==None:
            self.H[self.find_slot(key)].pushFront(key)
        else:
            self.H[self.find_slot(key)].search(key).value = value

    def remove(self, key):
        v = self.H[self.find_slot(key)].search(key)
        return self.H[self.find_slot(key)].remove(v)
